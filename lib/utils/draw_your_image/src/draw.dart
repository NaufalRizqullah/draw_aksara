...
class _DrawState extends State<Draw> {
  ...
  void _start(double startX, double startY, Offset p) {
  ...
  void _add(double x, double y, final p, {bool isEnd = true}) {
    if (isEnd) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: GestureDetector(
        onPanDown: (details) => _start(details.localPosition.dx,
            details.localPosition.dy, details.localPosition),
        onPanUpdate: (details) {
          _add(details.localPosition.dx, details.localPosition.dy,
              details.localPosition);
        },
        onPanEnd: (details) {
          _add(0, 0, null, isEnd: false);
        },
        child: LayoutBuilder(
...
class _FreehandPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final stroke in strokes) {
      ...
      for (int x = 0; x < stroke.points!.length - 1; x++) {
        if (stroke.points![x] != null && stroke.points![x + 1] != null) {
          // draw line
          canvas.drawLine(stroke.points![x]!, stroke.points![x + 1]!, paint);
        } else if (stroke.points![x] != null && stroke.points![x + 1] == null) {
          canvas.drawPoints(PointMode.points, [stroke.points![x]!], paint);
        }
      }
    }
